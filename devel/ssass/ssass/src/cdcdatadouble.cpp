...
CDCDataDouble::CDCDataDouble() : CDCData()
{
}


CDCDataDouble::~CDCDataDouble()
{
}


unsigned long CDCDataDouble::getSize( void ) const
{
	//die Größe dieses Bereichs zurückliefern
	unsigned long	_size = _array.size();
	unsigned char	_typeSize = sizeof( double );
...
CDCData::_typeId CDCDataDouble::getInternType( void ) const
{
	//Problem: Siehe COpcode-Klasse
	return( _CDCDataDouble );
}


void CDCDataDouble::insertElement( double elem )
{
...
std::ostream &operator<<( std::ostream &os, const CDCDataDouble &cls )
{
	//die Daten ausgeben
