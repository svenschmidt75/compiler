...
CDCDataSingle::CDCDataSingle() : CDCData()
{
}


CDCDataSingle::~CDCDataSingle()
{
}


unsigned long CDCDataSingle::getSize( void ) const
{
	//die Größe dieses Bereichs zurückliefern
	unsigned long	_size = _array.size();
	unsigned char	_typeSize = sizeof( float );
...
CDCData::_typeId CDCDataSingle::getInternType( void ) const
{
	//Problem: Siehe COpcode-Klasse
	return( _CDCDataSingle );
}


void CDCDataSingle::insertElement( float elem )
{
...
std::ostream &operator<<( std::ostream &os, const CDCDataSingle &cls )
{
	//die Daten ausgeben
